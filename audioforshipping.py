...
def handle_channel_conversion(audio_np, input_channels, output_channels):
    """Smart channel conversion with proper handling of all cases"""
    try:
...
def process_with_ai(audio_np, input_channels, output_channels, model, df_state):
    """Process audio through AI with smart channel handling and noise gating"""
    try:
        if input_channels == 1:
            mono_audio = audio_np
...
try:
    ...
    try:
        ...
        if input_info.get('maxInputChannels', 0) == 0:
    ...
    try:
        ...
        if output_info.get('maxOutputChannels', 0) == 0:
...
try:
    ...
    logger.info(f"  Default Low Output Latency: {input_info['defaultLowOutputLatency']:.3f}s")
...
try:
    ...
    logger.info("  - Ensure DeepFilterNet3/checkpoints/model_120.ckpt.best and config.ini are bundled with the executable.")
